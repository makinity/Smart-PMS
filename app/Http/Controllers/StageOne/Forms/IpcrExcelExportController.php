...
class IpcrExcelExportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $ws->mergeCells("A{$h1}:A{$h2}"); $ws->setCellValue("A{$h1}", 'OUTPUT');          $this->hdr($ws, "A{$h1}:A{$h2}");
        $ws->mergeCells("B{$h1}:B{$h2}"); $ws->setCellValue("B{$h1}", "Success Indicators\n(Measure + Target)"); $this->hdr($ws, "B{$h1}:B{$h2}");
        $ws->mergeCells("C{$h1}:C{$h2}"); $ws->setCellValue("C{$h1}", "6 Months Summary\nof Accomplishment"); $this->hdr($ws, "C{$h1}:C{$h2}");

        $ws->mergeCells("D{$h1}:G{$h1}"); $ws->setCellValue("D{$h1}", 'Rating');
        ...
        $ws->mergeCells("H{$h1}:H{$h2}"); $ws->setCellValue("H{$h1}", 'Remarks'); $this->hdr($ws, "H{$h1}:H{$h2}");

        $ws->mergeCells("I{$h1}:{$lastCol}{$h1}"); $ws->setCellValue("I{$h1}", 'Standards per Success Indicator');
        ...
        $ws->mergeCells("K{$r}:{$lastCol}{$r}"); $ws->setCellValue("K{$r}", 'Approved by:');
