...
Smart PMS ΓÇö a Philippine LGU Performance Management System built on Laravel 12 + Inertia.js + React 19. It digitizes the SPMS workflow (UWP ΓåÆ OPCR/IPCR planning ΓåÆ ORS monitoring ΓåÆ MPOR ΓåÆ accomplishment review ΓåÆ calibration), spread across five roles: `admin`, `pmt`, `dept-head`, `supervisor`, `employee`.
...
./vendor/bin/pint     # Format PHP (Laravel Pint, default ruleset ΓÇö no pint.json)
...
### Routing & roles
`routes/web.php` is the map of the whole app. It's organized into role-prefixed groups (`administrator`, `pmt`, `dept-head`, `supervisor`, `employee`), each guarded by `middleware(['auth', 'role:<role>'])` via **spatie/laravel-permission**. `/dashboard` is a role router that redirects to the correct role dashboard ΓÇö it also **auto-heals** a missing spatie role from the `users.role` column. When adding a feature, add its route inside the correct role group and create the matching controller under `app/Http/Controllers/<Role>/`.
...
### Layered: Controllers ΓåÆ Services ΓåÆ Models
...
Domain models carry explicit `status` constants that define their lifecycle. E.g. `UnitWorkPlan`: `draft ΓåÆ submitted ΓåÆ consolidated ΓåÆ endorsed ΓåÆ pmt_approved` (+ `returned`). OPCR/IPCR/MPOR/accomplishment models follow similar submit ΓåÆ review ΓåÆ approve/return chains across roles. Editability and locking are enforced by model helpers (e.g. `UnitWorkPlan::isEditableBySupervisor()`, `isLocked()`). Respect these status guards ΓÇö don't mutate records that the model considers locked.

### Notifications (real-time)
State transitions notify the next actor in the workflow. Use `WorkflowNotificationDispatcher` (`notifyUser` / `notifyRole`) rather than calling `$user->notify()` directly ΓÇö it skips inactive users. Notifications broadcast over **Reverb** (websocket) and render via `NotificationDropdown.jsx`; `BROADCAST_CONNECTION=reverb` and `QUEUE_CONNECTION=database`, so the queue worker and reverb server must be running for delivery.
...
- Shared Inertia props (`auth.user`, `flash`, `ziggy`) come from `HandleInertiaRequests::share` ΓÇö read user role/office from there client-side.
