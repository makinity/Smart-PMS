...
export default function Dashboard({ activePeriod, teamCount, uwpStatus, pendingReviews, endorsedCount, recentSubmissions, submissionsChart, statusChart }) {
        ...
        import('chart.js').then(({ Chart, registerables }) => {
            ...
            bar = new Chart(barRef.current, {
                ...
                data: {
                    ...
                    datasets: [{ data: statusChart.data, backgroundColor: 'rgba(59,130,246,0.6)', borderColor: 'rgba(59,130,246,1)', borderRadius: 6, borderWidth: 1 }],
                },
                options: chartOpts,
            });
        });
    ...
    const stats = [
        ...
        { label: 'Team Members',    value: teamCount,       caption: 'Employees under supervision' },
        { label: 'Pending Reviews', value: pendingReviews,  caption: 'Awaiting your endorsement' },
        { label: 'Endorsed',        value: endorsedCount,   caption: 'Forwarded to dept. head' },
    ];

    const quickLinks = [
        { title: 'Unit Work Plan',  desc: 'Manage your unit work plan',        href: '/supervisor/uwp',           icon: 'bi-kanban-fill' },
        { title: 'MPOR',            desc: 'Review team monthly reports',        href: '/supervisor/mpor',          icon: 'bi-file-text-fill' },
        { title: 'Accomplishment',  desc: 'Endorse employee submissions',       href: '/supervisor/accomplishment', icon: 'bi-award-fill' },
        { title: 'Team Tasks',      desc: 'Monitor team task progress',         href: '/supervisor/team-tasks',    icon: 'bi-people-fill' },
    ];
        ...
        <AppLayout title="Supervisor Dashboard">
            ...
            <div className="stack">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }} className="charts-row">
                    ...
                    <div style={{ ...card, minWidth: 0 }}>
                        <p style={cardHeader}>Submission Status Breakdown</p>
                        <div style={{ position: 'relative', height: 180, width: '100%' }}>
                            <canvas ref={barRef} />
                        </div>
                    </div>
...
const statusBadge = (s = '') => ({
    ...
    background: s.includes('done') || s.includes('approved') || s.includes('endorsed') || s.includes('released') ? 'rgba(34,197,94,0.12)' : s.includes('pending') || s.includes('submitted') ? 'rgba(234,179,8,0.12)' : 'rgba(59,130,246,0.12)',
    color: s.includes('done') || s.includes('approved') || s.includes('endorsed') || s.includes('released') ? '#4ade80' : s.includes('pending') || s.includes('submitted') ? '#facc15' : 'var(--admin-accent)',
