...
const FILTERS = [
    ...
    { key: 'supervisor_recommended', label: 'Recommended' },
];
...
export default function Index() {
    ...
    return (
        <AppLayout title="Team IDP" description="Review employee Individual Development Plans">
            <div style={{ ...card, padding: '1.25rem' }}>

                {/* Header */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: 8 }}>
                    <div>
                        <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--admin-text-primary)' }}>Team IDPs</div>
                ...
                {filtered.length === 0 ? (
                    ...
                    const sc = STATUS_CFG[p.status] ?? STATUS_CFG.submitted;
                    const isPending = p.status === 'submitted';
                    const ratingColor = RATING_COLOR[p.source_rating] ?? '#ef4444';
                    return (
                        <div key={p.id}
                            onClick={() => router.visit(`/supervisor/idp/${p.id}`)}
                            style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap', minWidth: 0 }}>
                                        ...
                                        <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '1px 8px', borderRadius: 99,
                                            color: sc.c, background: sc.bg, border: `1px solid ${sc.bc}`, flexShrink: 0 }}>{sc.label}</span>
                                    </div>
