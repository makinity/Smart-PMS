...
export default function Show() {
    ...
    const sectionLabel = {
        ...
        letterSpacing: '0.08em', color: 'var(--admin-text-muted)', marginBottom: '0.5rem',
    };

    async function handleApprove() {
        if (!await confirm('Approve this accomplishment submission?')) return;
        setApproving(true);
        router.post(`/dept-head/accomplishment-review/${submission.id}/approve`, {}, {
            preserveScroll: true,
    ...
    return (
        <AppLayout title="Review Accomplishment" description={`${submission?.employee_name} — ${submission?.period}`}>
            ...
            {/* All sections in a flex column with consistent 1rem gap */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 0 }}>
                                    ...
                                    <div style={{ minWidth: 0 }}>
                                        <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--admin-text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{submission?.employee_name}</div>
                    ...
                    <div style={{ padding: '1rem 1.25rem' }}>
                        ...
                        {status === 'returned_to_employee' && submission?.dept_head_remarks && (
                            <div style={{ marginTop: '0.75rem', padding: '0.65rem 0.9rem', borderRadius: 8, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', fontSize: '0.82rem', color: '#f87171' }}>
                                <strong>Return remarks:</strong> {submission.dept_head_remarks}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                    ...
                    <div style={{ ...card, padding: '1.1rem 1.25rem' }}>
                        <div style={{ marginBottom: '0.65rem' }}>
                            <div style={sectionLabel}>IPCR SCORE</div>
                        </div>
                        {ipcrMeta ? (
                            <div style={{ marginBottom: '0.65rem' }}>
                ...
                <div style={{ ...card, padding: '1.1rem 1.25rem' }}>
                    ...
                    {activeTab === 'smpor'
                        ? <SmporTable table={smporTable} />
                        : <IpcrSections sections={ipcrSections} ipcrMeta={ipcrMeta} />
                    }
                </div>
                ...
                {/* ── Supervisor remarks (read-only for dept head) ── */}
