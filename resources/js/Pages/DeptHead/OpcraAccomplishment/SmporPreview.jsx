...
export default function SmporPreview() {
    const { submission, table } = usePage().props;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ background: 'var(--admin-card)', border: '1px solid var(--admin-border-strong)', borderRadius: 'var(--admin-radius)', padding: '1rem 1.25rem' }}>
                    ...
                    <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--admin-text-primary)', marginTop: 8 }}>
                        SMPOR — {submission?.employee_name}
                    </div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                ...
                                <tbody>
                                    {section.rows.map((row, i) => (
                                        <tr key={i}>
                                            <td style={{ ...td, position: 'sticky', left: 0, zIndex: 1, background: 'var(--admin-card)', fontWeight: 500 }}>{row.output}</td>
