...
const SUBMISSION_STATUS = {
    ...
    supervisor_approved:     { label: 'Approved',        c: '#a78bfa', bg: 'rgba(167,139,250,0.12)' },
...
function ratingCfg(r) {
    return RATING_CFG[r] ?? { color: 'var(--admin-text-muted)', bg: 'var(--admin-bg-secondary)', icon: '' };
}
...
export default function Index() {
    ...
    return (
        <AppLayout title="My Performance History" description="Your performance records across all periods">
                        ...
                        {ipcrs.length === 0 ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', padding: '0.75rem' }}>
                                {ipcrs.map((ipcr, i) => {
                                    ...
                                    return (
                                        <div key={ipcr.id} style={{ border: '1px solid var(--admin-border)', borderRadius: 'var(--admin-radius)',
                                            padding: '0.85rem 1rem', background: i % 2 === 0 ? 'transparent' : 'var(--admin-bg-secondary)',
                                            cursor: 'pointer' }}
                                            onClick={() => router.visit(`/employee/history/ipcr/${ipcr.id}`)}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                                                <span style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--admin-text-primary)' }}>{ipcr.period}</span>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.4rem 1rem', fontSize: '0.78rem' }}>
                                                <div>
                                                    <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'var(--admin-text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>Status</div>
                                                    <Badge label={ipcr.status?.replace(/_/g, ' ')} color="var(--admin-text-muted)" bg="var(--admin-bg-secondary)" />
                                                </div>
                                                <div>
                                                    <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'var(--admin-text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>Final Score</div>
                                                    <span style={{ fontWeight: 700 }}>{ipcr.final_score?.toFixed(2) ?? '—'}</span>
                                                </div>
                                                <div>
                                                    <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'var(--admin-text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>PMT Score</div>
                                                    <span style={{ fontWeight: 700, color: 'var(--admin-accent)' }}>{ipcr.pmt_score ? ipcr.pmt_score.toFixed(2) : '—'}</span>
                                                </div>
                                                <div>
                                                    <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'var(--admin-text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>Committed</div>
                                                    <span style={{ color: 'var(--admin-text-muted)' }}>{ipcr.committed_at ?? '—'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    );
                                        ...
                                        <tr style={{ borderBottom: '1px solid var(--admin-border)', background: 'var(--admin-bg-secondary)' }}>
                                            {['Period', 'Status', 'Final Score', 'PMT Score', 'Rating', 'Committed'].map(h => (
                                                <th key={h} style={{ padding: '0.65rem 1rem', textAlign: 'left', fontWeight: 700,
                                                    fontSize: '0.7rem', color: 'var(--admin-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {ipcrs.map((ipcr, i) => {
                                            ...
                                            const adjCfg = ratingCfg(adj);
                                            return (
                                                <tr key={ipcr.id} onClick={() => router.visit(`/employee/history/ipcr/${ipcr.id}`)}
                                                    style={{ borderBottom: '1px solid var(--admin-border)', background: i % 2 === 0 ? 'transparent' : 'var(--admin-bg-secondary)', cursor: 'pointer' }}>
