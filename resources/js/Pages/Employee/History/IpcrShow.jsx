        ...
        return (
            <>
                ...
                <div style={{ position: 'fixed', bottom: 0, left: sidebarLeft, right: 0, zIndex: 1101,
                    ...
                    boxShadow: '0 -8px 32px rgba(0,0,0,0.3)', maxHeight: '80vh', display: 'flex', flexDirection: 'column',
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '0.75rem 1.25rem 1.5rem' }}>{table}</div>
                </div>
    ...
    return (
        <>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.25rem' }}>{table}</div>
            </div>
        </>
    );
}
...
function RatingBadge({ label, value }) {
    ...
    return (
        <div style={{ textAlign: 'center', minWidth: 44 }}>
            ...
            <div style={{ fontSize: '0.95rem', fontWeight: 800, color, lineHeight: 1 }}>{value != null ? value.toFixed(2) : '—'}</div>
        </div>
    );
}
        ...
        <div style={{ borderBottom: '1px solid var(--admin-border)', padding: '0.75rem 1rem' }}>
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 180 }}>
                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--admin-text-primary)', lineHeight: 1.45, marginBottom: 3 }}>{ind.indicator_text}</div>
                    {ind.target_timeline && (
                ...
                <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', flexShrink: 0 }}>
                    {[['Q', Q], ['E', E], ['T', T], ['A', A]].map(([l, v]) => <RatingBadge key={l} label={l} value={v} />)}
            ...
            {showStd && <StandardsOverlay standards={ind.standards} title={ind.indicator_text} onClose={() => setShowStd(false)} bp={bp} sidebarLeft={sidebarLeft} />}
        </div>
    );
}
...
function MfoAccordion({ mfo, bp, sidebarLeft }) {
    ...
    return (
        <div style={{ border: '1px solid var(--admin-border)', borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
            <button onClick={() => setOpen(v => !v)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '0.7rem 1rem',
            ...
            {open && (
                <div style={{ background: 'var(--admin-card)' }}>
                    {mfo.indicators.map((ind, i) => <IndicatorRow key={ind.id} ind={ind} index={i + 1} bp={bp} sidebarLeft={sidebarLeft} />)}
                </div>
            )}
        </div>
    );
...
export default function IpcrShow() {
    ...
    return (
        <AppLayout title="IPCR Detail" description={`Period: ${period?.name}`}>
                ...
                <div style={{ position: 'sticky', top: 0, zIndex: 40, background: 'var(--admin-card)', borderBottom: '1px solid var(--admin-border)', padding: '0.6rem 1rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                            <button onClick={() => router.visit('/employee/history')}
                            ...
                            <div style={{ minWidth: 0 }}>
                                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--admin-text-primary)' }}>IPCR Detail</div>
                                <div style={{ fontSize: '0.72rem', color: 'var(--admin-text-muted)' }}>{employee?.name} · {employee?.office} · {period?.name}</div>
                            </div>
                        </div>
                        {score > 0 && (
                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                <div style={{ fontSize: '1.2rem', fontWeight: 800, color: scoreColor, lineHeight: 1 }}>{score.toFixed(2)}</div>
                                <div style={{ fontSize: '0.62rem', color: 'var(--admin-text-muted)' }}>{rating ?? 'Score'}</div>
                            </div>
                        )}
                    </div>
                </div>
                <div style={{ padding: '0.6rem 1rem', display: 'flex', gap: '1.5rem', flexWrap: 'wrap', fontSize: '0.72rem', color: 'var(--admin-text-muted)' }}>
