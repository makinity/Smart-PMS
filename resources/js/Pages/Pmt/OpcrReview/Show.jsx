...
export default function Show() {
    ...
    const sc    = STATUS[opcr?.status] ?? STATUS.submitted;
    ...
    const sc2       = STATUS[status] ?? STATUS.submitted;
        ...
        <AppLayout title="OPCR Review">
            ...
            <div style={s.topbar}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <button onClick={() => router.visit('/pmt/opcr-review')} style={s.backBtn}>&#8592;</button>
                    <div style={s.divider} />
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={s.period}>{opcr?.period}</span>
                        <span style={s.office}>{opcr?.office}</span>
                    </div>
                    <div style={{ ...s.statusPill, background: sc2.bg, color: sc2.color, border: `1px solid ${sc2.border}` }}>
                        <span>{sc2.icon}</span><span>{sc2.label}</span>
                    </div>
                </div>
            ...
            {/* ── Return remarks banner (when returned) ── */}
            ...
            <div style={s.layout} className="opcr-layout">
                ...
                <aside style={s.leftPanel} className="opcr-left">
                    ...
                    {uwps.map(u => (
                        ...
                        >
                            <div style={{ ...s.avatar, background: colorFor(u.supervisor) }}>{initials(u.supervisor)}</div>
            ...
            {returnOpen && (
                <div style={sm.overlay} onClick={() => setReturnOpen(false)}>
                    <div style={{ ...sm.modal, width: 420 }} onClick={e => e.stopPropagation()}>
                        <div style={sm.header}>
                            <span style={sm.title}>Return OPCR — Add Remarks</span>
                        ...
                        <div style={{ padding: '1.25rem' }}>
                            ...
                            <textarea value={remarks} onChange={e => setRemarks(e.target.value)}
                                placeholder="e.g. Some indicators are missing QET standards…"
                                rows={4}
            ...
            <div style={{ paddingLeft: '2.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {(si.target_quantity || si.target_timeline) && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                        <span style={s.detailLabel}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg> Target</span>
...
function AssigneeStack({ assignees }) {
    ...
    return (
        <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }} onClick={() => setOpen(true)}>
                <div style={{ display: 'flex' }}>
                    {visible.map((a, i) => (
                        <div key={i} style={{ ...sm.avatar, background: colorFor(a.employee?.name), marginLeft: i === 0 ? 0 : -8, zIndex: MAX - i }}>{initials(a.employee?.name)}</div>
                    ))}
                    {extra > 0 && <div style={{ ...sm.avatar, background: 'var(--admin-bg-secondary)', color: 'var(--admin-text-muted)', border: '2px solid var(--admin-border)', marginLeft: -8, fontSize: '0.6rem', fontWeight: 700 }}>+{extra}</div>}
                </div>
            ...
            {open && (
                <div style={sm.overlay} onClick={() => setOpen(false)}>
                    <div style={sm.modal} onClick={e => e.stopPropagation()}>
                        <div style={sm.header}><span style={sm.title}>Assigned Employees</span><button style={sm.closeBtn} onClick={() => setOpen(false)}>✕</button></div>
                        <div style={sm.list}>
                            {assignees.map((a, i) => (
                                <div key={i} style={sm.row}>
                                    <div style={{ ...sm.bigAvatar, background: colorFor(a.employee?.name) }}>{initials(a.employee?.name)}</div>
                                    <div><div style={sm.empName}>{a.employee?.name ?? '—'}</div>{a.employee?.position && <div style={sm.empPos}>{a.employee.position}</div>}</div>
                                </div>
                            ))}
                        </div>
                    </div>
...
const sm = {
    avatar:    { width: 30, height: 30, borderRadius: '50%', color: '#fff', fontSize: '0.62rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--admin-card)', flexShrink: 0 },
