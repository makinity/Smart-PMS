...
export default function Show() {
    ...
    function handleApprove() {
        ...
        router.patch(`/pmt/opcr-review/${opcr.id}/approve`, {}, {
            ...
            onError:   () => toast('Failed to approve OPCR.', 'error'),
    ...
    function submitReturn() {
        ...
        setLoading('return');
        router.patch(`/pmt/opcr-review/${opcr.id}/return`, { remarks }, {
            onSuccess: () => { setStatus('returned'); setReturnOpen(false); setRemarks(''); toast('OPCR returned to dept. head.', 'info'); },
            onError:   () => toast('Failed to return OPCR.', 'error'),
            onFinish:  () => setLoading(null),
        });
    }
    ...
    return (
        <AppLayout title="OPCR Review">
            ...
            <div style={{ borderRadius: 'var(--admin-radius-lg)', border: '1px solid var(--admin-border-strong)', background: 'var(--admin-card)', boxShadow: 'var(--admin-shadow)', overflow: 'clip' }}>
                ...
                <div style={{ position: 'sticky', top: 0, zIndex: 40, background: 'var(--admin-card)', borderBottom: '1px solid var(--admin-border)', padding: '0.6rem 1rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                            <button onClick={() => router.visit('/pmt/opcr-review')} style={s.backBtn}>
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
                            </button>
                            <div style={{ width: 1, height: 28, background: 'var(--admin-border-strong)', flexShrink: 0 }} />
                            <div style={{ minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                                    <span style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--admin-text-primary)', whiteSpace: 'nowrap' }}>{opcr?.period ?? 'OPCR Review'}</span>
                                ...
                                <div style={{ fontSize: '0.72rem', color: 'var(--admin-text-muted)', marginTop: '0.1rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{opcr?.office ?? ''}</div>
                            </div>
                        </div>
                        {bp !== 'mobile' && <Actions />}
                    </div>
                ...
                {status === 'returned' && <ReturnRemarksBanner remarks={opcr?.return_remarks} label="Returned to Dept. Head" />}

                {/* Tablet/Mobile: two-row tab nav + Contributors button */}
                {bp !== 'desktop' && (
                ...
                <div style={{ display: 'flex', flexDirection: bp !== 'desktop' ? 'column' : 'row', minHeight: 600 }}>
                            ...
                            <div style={{ padding: '0.5rem 0' }}>
                                ...
                                {uwps.map(u => (
                                    <button key={u.id}
                                        ...
                                        <span style={u.status === 'approved' ? s.approvedDot : s.pendingDot}>{u.status === 'approved' ? '✓' : '✗'}</span>
                                    </button>
                                ))}
                    ...
                    <main style={{ ...s.centerPanel, paddingBottom: bp === 'mobile' ? '5rem' : '1.5rem' }}>
                        {mfosToShow.length === 0 && !displayFn && <div style={s.empty}>Select a function to view its indicators.</div>}
                        {mfosToShow.map(mfo => (
                            <section key={mfo.id} style={s.mfoGroup}>
                                <div style={s.mfoHeader}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        <span style={s.mfoIcon}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></span>
                    ...
                    <div style={sm.modal} onClick={e => e.stopPropagation()}>
                        <div style={sm.header}>
                            <div>
                                ...
                                <div style={{ fontSize: '0.72rem', color: allApproved ? '#4ade80' : '#facc15', marginTop: 2 }}>{approvedCount} of {uwps.length} UWPs approved</div>
                            </div>
                            ...
                            {uwps.map(u => (
                                <button key={u.id}
                                    ...
                                    <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                                        ...
                                        {u.mfo_labels && <div style={{ fontSize: '0.72rem', color: 'var(--admin-text-muted)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.mfo_labels}</div>}
                                    </div>
                                    <span style={u.status === 'approved' ? s.approvedDot : s.pendingDot}>{u.status === 'approved' ? '✓' : '✗'}</span>
                                </button>
                            ))}
                            {activeUwpId && (
                                <button style={sm.clearBtn} onClick={() => { setActiveUwpId(null); setContribOpen(false); }}>Show all UWPs</button>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {returnOpen && (
                <div style={sm.overlay} onClick={() => setReturnOpen(false)}>
                    <div style={{ ...sm.modal, maxWidth: 420 }} onClick={e => e.stopPropagation()}>
                        <div style={sm.header}>
                            <span style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--admin-text-primary)' }}>Return OPCR — Add Remarks</span>
                        ...
                        <div style={{ padding: '1.25rem' }}>
                            <p style={{ fontSize: '0.82rem', color: 'var(--admin-text-muted)', marginBottom: '0.75rem' }}>
                                Please state the reason for returning this OPCR to the department head.
                            </p>
                            <textarea value={remarks} onChange={e => setRemarks(e.target.value)}
                                placeholder="e.g. Some indicators are missing QET standards…" rows={4}
...
const s = {
    ...
    approveBtn:   { display: 'flex', alignItems: 'center', gap: '0.45rem', padding: '0.45rem 1.1rem', borderRadius: 8, border: 'none', background: 'rgba(5,150,105,0.9)', color: '#fff', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' },
    returnBtn:    { display: 'flex', alignItems: 'center', gap: '0.45rem', padding: '0.45rem 1.1rem', borderRadius: 8, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.1)', color: '#f87171', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' },
    ...
    avatar:       { width: 26, height: 26, borderRadius: '50%', color: '#fff', fontSize: '0.6rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--admin-card)', marginLeft: -6, flexShrink: 0 },
