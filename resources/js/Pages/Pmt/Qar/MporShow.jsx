...
function Avatar({ name, src, size = 44 }) {
    ...
    return <div style={{ width: size, height: size, borderRadius: '50%', background: 'rgba(59,130,246,0.15)', color: 'var(--admin-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: size * 0.33, flexShrink: 0 }}>{initials}</div>;
}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 880 }}>
                ...
                <tbody>
                    ...
                    <tr style={{ background: 'rgba(59,130,246,0.06)', borderTop: '2px solid var(--admin-accent)' }}>
                        ...
                        {W.map(w => <td key={w} style={{ padding: '0.5rem 0.4rem', textAlign: 'center', fontWeight: 600, borderLeft: w===1?'1px solid var(--admin-border)':undefined }}>{grandQty[w]||0}</td>)}
...
export default function MporShow() {
    const { mpor, employee, sections, grandQty, grandQualAvg, grandTimeAvg, grandQtyTotal, backQarId } = usePage().props;
    ...
    return (
        <AppLayout title={`QAR MPOR — ${employee?.name}`}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>

                <div style={card}>
                    <div style={{ marginBottom: '1rem' }}>
                        <a href={backQarId ? `/pmt/qar/${backQarId}` : '/pmt/qar'} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', color: 'var(--admin-text-muted)', textDecoration: 'none', fontWeight: 600 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile?'1fr 1fr':'repeat(3,1fr)', gap: '0.65rem' }}>
                    <div style={card}><p style={statLabel}>Total Outputs</p><div style={{ fontSize: '1.7rem', fontWeight: 800, color: 'var(--admin-text-primary)', lineHeight: 1 }}>{grandQtyTotal}</div></div>
                    <div style={card}><p style={statLabel}>Quality Score</p><div style={{ fontSize: '1.7rem', fontWeight: 800, color: '#22c55e', lineHeight: 1 }}>{grandQualAvg||'—'}</div></div>
                    <div style={{ ...card, gridColumn: isMobile?'span 2':'auto' }}><p style={statLabel}>Timeliness Score</p><div style={{ fontSize: '1.7rem', fontWeight: 800, color: '#f59e0b', lineHeight: 1 }}>{grandTimeAvg||'—'}</div></div>
                </div>

                <div style={card}>
                    <p style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--admin-text-primary)', marginBottom: '1rem' }}>Monthly Performance Records</p>
                    {grandQtyTotal === 0 ? (
                        <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--admin-text-muted)', fontSize: '0.88rem' }}>No rated entries for {monthLabel}.</div>
                    ) : isMobile ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile?'1fr':'1fr 1fr', gap: '0.65rem' }}>
                    ...
                    <div style={{ ...card, borderLeft: '3px solid #22c55e' }}>
                        <p style={statLabel}>Supervisor Approval</p>
